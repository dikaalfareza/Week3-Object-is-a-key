...
function countProfit(shoppers) {
  ...
  for (let a of listBarang) {
    let obj = { product: a[0], shoppers: [], leftOver: a[2], totalProfit: 0 };

    for (let b of shoppers) {
      if (b.product === obj.product) {
        if (obj.leftOver >= b.amount) {
          obj.leftOver -= b.amount;
          obj.shoppers.push(b.name);
          obj.totalProfit += b.amount * a[1];
        }
      }
    }
